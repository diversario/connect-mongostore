...
function getStore(Store) {
  ...
  function MongoStore (options) {
    ...
    if (typeof options == 'string') {
      this.mongoClient = new MongoClient(options)
    } else {
      this.mongoClient = setupDb.call(this, this.options)
    }
  }
  ...
  MongoStore.prototype.getDatabase = function (cb) {
    ...
    if (this.db) {
      ...
      self.mongoClient.open(function (err, mongoClient) {
        ...
        var db = mongoClient.db(self.databaseName)
  ...
  MongoStore.prototype.getCollection = function (cb) {
    ...
    self.openCalled = self.openCalled || self.db && self.db.openCalled

    if (!self.openCalled) return self.getDatabase(cb)
...
function setupDb(opts) {
  if (opts.connectionString) {
    this.databaseName = opts.db
    return new MongoClient(opts.connectionString)
  }

  if (opts.mongooseConnection) return dbFromMongooseConnection.call(this, opts.mongooseConnection, opts)
  if (!opts.db) throw new Error('Required MongoStore option `db` missing')
  if (typeof opts.db == 'object' && opts.db.databaseName) {
    this.db = opts.db
    return null
  } // Assume it's an instantiated DB Object
  ...
  if (Array.isArray(opts.db.servers)) {
    ...
    this.databaseName = opts.db.name

    return new MongoClient(new mongo.ReplSetServers(serverArray), opts.db.replicaSetOptions || {'w': 1})
  }
...
function dbFromMongooseConnection(mongooseConnection, opts) {
  ...
  this.databaseName = mongooseConnection.db.databaseName
