...
function parseUrl(path) {
  ...
  var db_url = url.parse(path);

  if (db_url.port) {
    parsed.port = parseInt(db_url.port);
  }
...
function setupDb(opts) {
  if (opts.mongoose_connection) return dbFromMongooseConnection(opts.mongoose_connection, opts)
  ...
  if (Array.isArray(opts.db.servers)) {
    ...
    this.db = new mongo.Db(opts.db.name, new mongo.ReplSetServers(serverArray), opts.db.replicaSetOptions || {w: 0});
  }
